...
RUN apt-get update ; \
    apt-get install -y build-essential wget cmake libx11-dev libopenblas-dev shtool unzip && \
...
RUN apt-get update && \
    apt-get install -y wget unzip nodejs npm aria2 python3-pip supervisor \&& \
    rm -rf /var/lib/apt/lists/*
